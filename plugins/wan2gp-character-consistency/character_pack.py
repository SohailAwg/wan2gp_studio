...
def _base_prompt(identity_prompt: str, shot_prompt: str, style_prompt: str = "") -> str:
    parts = [
    ...
    parts.append(
        ...
        "Do not redesign the character. Keep lighting and camera movement cinematic and temporally coherent."
    )
    return "\n".join(parts)
...
def build_settings(
    *,
    ...
    prompt = _base_prompt(identity_prompt, shot_prompt, style_prompt)
...
def research_summary_markdown() -> str:
    return (
        "Commercial character tools use three layers: curated multi-angle references, an identity memory/training layer, "
        "and shot-by-shot prompting that reuses the same identity anchor. In WanGP, the closest open workflow is to save "
        "a character pack and re-inject it through Bernini, VACE Stand-In, Stand-In, SCAIL-2, or JoyAI-Echo depending on the shot."
    )
