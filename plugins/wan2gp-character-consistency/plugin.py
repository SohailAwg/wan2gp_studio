...
PlugIn_Name = "Character Consistency Studio"
...
class CharacterConsistencyPlugin(WAN2GPPlugin):
    def __init__(self):
        ...
        self.version = "1.0.0"
        self.description = "Create reusable character packs and WanGP settings for reference-consistent character video."
    ...
    def create_config_ui(self, api_session):
        ...
        def _build_payload(character_name, mode, identity_prompt, negative_prompt, style_prompt, shot_prompts_text, path_text, uploaded_files, resolution, video_length, seed, control_video):
        ...
        def preview_settings(character_name, mode, identity_prompt, negative_prompt, style_prompt, shot_prompts_text, path_text, uploaded_files, resolution, video_length, seed, control_video):
        ...
        def export_pack(character_name, mode, identity_prompt, negative_prompt, style_prompt, shot_prompts_text, path_text, uploaded_files, resolution, video_length, seed, control_video):
            pack, issues = _build_payload(character_name, mode, identity_prompt, negative_prompt, style_prompt, shot_prompts_text, path_text, uploaded_files, resolution, video_length, seed, control_video)
        ...
        def export_settings(character_name, mode, identity_prompt, negative_prompt, style_prompt, shot_prompts_text, path_text, uploaded_files, resolution, video_length, seed, control_video):
            pack, issues = _build_payload(character_name, mode, identity_prompt, negative_prompt, style_prompt, shot_prompts_text, path_text, uploaded_files, resolution, video_length, seed, control_video)
            blocking = [issue for issue in issues if issue.startswith("Add ")]
            if blocking:
                raise gr.Error("\n".join(blocking))
        ...
        with gr.Column():
            gr.Markdown("## Character Consistency Studio")
            ...
            with gr.Row():
                with gr.Column(scale=5):
                    ...
                    negative_prompt = gr.Textbox(
                        label="Negative identity drift prompt",
                        value="different person, face drift, changed hairstyle, changed age, inconsistent outfit, distorted hands, extra fingers, broken anatomy",
                        lines=2,
                    )
                    style_prompt = gr.Textbox(label="Optional locked style/world prompt", value="", lines=2)
